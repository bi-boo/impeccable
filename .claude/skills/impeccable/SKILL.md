---
name: impeccable
description: "用户做前端 UI 任务时使用——网页/落地页/dashboard/APP 原型/组件/表单/空状态/引导流程/HTML 汇报/演示稿。常见中文意图→对应 sub-command：\"做一个/搭一版/写个原型/帮我做\"→craft；\"先想想怎么做/帮我规划下/上手前理一下\"→shape；\"评审下/有什么问题/体验上行不行/UX review\"→critique；\"技术质量过一遍/可访问性/响应式/性能审查\"→audit；\"上交前打磨/收尾/对齐设计系统/最后过一遍\"→polish；\"太花了/太吵了/收敛点/柔和点/太骚了\"→quieter；\"太朴素/太单调/像 wireframe/强烈点/醒目点\"→bolder；\"砍掉一切非必要/极简/提炼/化繁为简\"→distill；\"加点颜色/太单色/死气沉沉/太灰\"→colorize；\"加点动效/加微互动/动起来\"→animate；\"加点惊喜/太严肃/加点趣味/有点意思\"→delight；\"上技术炫技/做出贵的感觉/惊艳一点\"→overdrive；\"字看不清/标题不突出/字号乱/字体丑\"→typeset；\"太挤/留白不舒服/间距乱/对不齐\"→layout；\"文案不知所云/改文案/话讲不清\"→clarify；\"手机上看烂了/跨设备适配/移动端\"→adapt；\"卡/性能优化/加载慢\"→optimize；\"文本溢出/错误态/空状态/i18n/边界场景\"→harden；\"首次使用引导/激活路径/新手引导\"→onboard；\"浏览器实时改/看效果/在浏览器里调\"→live；\"反推现有项目设计系统/反向生成 DESIGN.md\"→document；\"提取组件到设计系统/抽公共组件\"→extract；\"首次设置/收集设计偏好/初始化项目设计上下文\"→teach。不适用于：纯后端/纯 CLI/不涉及 UI 任务。"
version: 3.0.6
user-invocable: true
